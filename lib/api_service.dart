...
class ApiService {
  ...
  // 🆕 DIAGNOSTIC TEMPORAIRE — envoie un message texte au serveur, qui
  // l'écrit dans son journal d'erreurs avec le préfixe [FCM][iOS-diag].
  // Sert uniquement à tracer le déroulement de FcmService.initialiser()
  // sur iOS sans accès à la console Xcode. À retirer une fois le
  // blocage identifié.
  static Future<void> debugLog(String msg) async {
    try {
      await _post('debug_log', {"msg": msg});
    } catch (_) {} // ne jamais faire planter l'app pour un simple log
  }

  // 🆕 CORRIGÉ — cette fonction renvoyait void et avalait toute erreur
  // en silence (catch (_) {}), ce qui empêchait de savoir si l'envoi du
  // jeton au serveur avait réellement réussi. Elle renvoie maintenant la
  // réponse du serveur (ou un statut d'erreur explicite en cas d'échec
  // réseau), pour que FcmService puisse la tracer dans ses logs [FCM].
  static Future<Map<String, dynamic>> saveFcmToken(String telephone, String token) async {
    try {
      return await _post('save_fcm_token', {"telephone": telephone, "fcm_token": token});
    } catch (e) {
      return {"status": "error", "message": "Echec reseau : $e"};
    }
  }
